          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-7 sticky top-24">
              <div className="flex flex-col items-center text-center mb-6">
                ...
                <h1 className="text-xl font-bold text-slate-900 mb-1">{userProfile.name}</h1>
                <p className="text-slate-500 text-sm mb-2">{userProfile.email}</p>
                {userProfile.createdAt && (
                  <p className="text-slate-400 text-xs mb-5">Joined {new Date(userProfile.createdAt).toLocaleDateString()}</p>
                )}
                
                <button
                  onClick={() => navigate('/dashboard')}
                  className="w-full inline-flex justify-center items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-colors duration-200 border border-slate-300 text-slate-700 hover:bg-slate-50 shrink-0"
                >
